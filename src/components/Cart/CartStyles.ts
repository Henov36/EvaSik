...
export const CartRow = styled.li`
	...
	grid-template-areas: "img info price remove";
	align-items: center;
	gap: 14px;
	padding-bottom: 18px;
...
export const CartTotalRow = styled.div<{ $emphasis?: boolean }>`
	display: flex;
	justify-content: space-between;
	font-size: ${({ $emphasis }) => ($emphasis ? "17px" : "15px")};
	color: #4a5a41;

	strong {
		color: #204a1f;
		font-size: ${({ $emphasis }) => ($emphasis ? "20px" : "inherit")};
	}
`;
